"""Offline, deterministic tests for the improvement scan.

The scan is PURE and deterministic: no model calls, no network, no randomness, no
clock reads beyond row timestamps. Each test seeds a recorded session
+ judgment(s) through the public store API, runs ``scan_session`` over the log, and
asserts on the persisted ``log_improvement`` rows read back through the read surface.
...
vocabularies and the parsed docs — no ``if criterion_id == ...`` ladders. The
...
def test_load_playbook_exposes_version_and_required_areas() -> None:
    """The playbook parses into a version + the six required area ids (four
...
def test_load_playbook_fails_loudly_on_missing_required_area(
    ...
    """A playbook missing a required area fails loudly — code never silently
...
def test_load_playbook_fails_loudly_on_missing_version(
    ...
    """A playbook with no ``playbook_version`` header fails loudly."""
    text = improvement._read_playbook_text()
...
def test_weak_criterion_yields_one_proposal_in_mapped_area(tmp_path: Path) -> None:
    """A criterion banded weak yields one proposal in the area mapped from
...
def test_strong_adequate_not_applicable_yield_nothing(tmp_path: Path) -> None:
    """Strong / adequate / not_applicable bands produce no proposal."""
    log_path = tmp_path / "session_log.duckdb"
    conn = _open_initialized(log_path)
    sid = _new_session(conn)
    _record_judgment(
...
def test_non_complete_status_yields_harness_reliability(tmp_path: Path) -> None:
    """A judgment whose status is not ``complete`` yields exactly one
...
def test_off_rubric_criterion_yields_rubric_drift(tmp_path: Path) -> None:
    """A judged criterion id the current rubric does not define yields one
...
def test_multiple_weak_criteria_each_yield_a_proposal(tmp_path: Path) -> None:
    """Two weak criteria in different categories yield two proposals in their
    respective areas."""
    log_path = tmp_path / "session_log.duckdb"
    conn = _open_initialized(log_path)
    sid = _new_session(conn)
...
def test_scan_carries_playbook_version_on_each_proposal(tmp_path: Path) -> None:
    """Each proposal records the playbook_version that produced it."""
    log_path = tmp_path / "session_log.duckdb"
    conn = _open_initialized(log_path)
    sid = _new_session(conn)
    _record_judgment(
...
def test_rescan_is_idempotent(tmp_path: Path) -> None:
    """Re-running the scan over the same judgments writes nothing new and
...
def test_scan_no_judgments_writes_nothing(tmp_path: Path) -> None:
    """A session with no judgments yields no proposals (nothing to
