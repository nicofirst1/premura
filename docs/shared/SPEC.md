...
>
> Companion to [DOCTRINE.md](DOCTRINE.md) (product stance), [../history/architecture/ARCHITECTURE_HISTORY.md](../history/architecture/ARCHITECTURE_HISTORY.md) (implementation history), [USERJOURNEY.md](../using/USERJOURNEY.md) (experience), [STATUS.md](STATUS.md) (what works today), and [ROADMAP.md](ROADMAP.md) (what's next). This document is the source of truth for **what the system must do**, not how.
...
- A Python CLI (`premura`) covering setup, ingest, status, export, opt-in upload, doctor, monthly operation, garbage collection, launchd installation, bundled skill installation, and bounded profile capture.
...
| FR-7  | The system SHALL upload encrypted artifacts to `gdrive:/backups/premura/YYYY/MM/` only when the user explicitly invokes the opt-in upload command, and verify via `rclone lsl`.                                                             | Run `premura upload --month YYYY-MM`; verify listing shows expected files with matching sizes.                                                                                                |
...
- The DuckDB warehouse is the **system of record** post-ingestion. Stage/raw files MAY be garbage-collected per `premura gc --keep N` (default N=3 months).
...
- `premura doctor` MUST report status of: `age`, `rclone`, `uv`, DuckDB file presence, age key fingerprint, rclone remote reachability, free disk in `data/`.
