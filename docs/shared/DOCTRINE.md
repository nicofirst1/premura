...
**Single home per durable fact.** Every durable fact has exactly one home; everywhere else links to it instead of restating it, because a restated fact is a future stale fact. A count or inventory of what is shipped (tool counts, signal lists, test totals) lives at its live source in the code (`premura --help`, `premura status`, `engine.list_*()`, the pinned inventory tests). A contract, rubric, or decision lives in its `ALL_CAPS.md` or ADR; prose points there rather than paraphrasing it. The self-check, applied whenever a doc is written or reviewed alongside the altitude test above: _does this doc restate a fact that already has a home?_

**No provenance-dating in living prose.** Living docs describe the current state, not how it got there. Do not append inline dates, session tags, or status headers to prose ("as of 2026-07", "raised in review", "Status: synthesized from the intake mission"). Who-and-when belongs only where it is the point: append-only mission narratives in [CHANGELOG.md](CHANGELOG.md), ADRs, and git history. Keep a date in living prose only when the date itself is load-bearing (a deadline, a retention window, a version boundary). The self-check: _if this line were true forever, would the date still earn its place?_ Provenance dating is how a doc silently rots: the date freezes while the fact moves on. This is also why the mission-citation gate (`ops/check_no_mission_citations.sh`) rejects FR-/WP##/T### tokens in tracked code and docs, keeping only durable refs (ADR NNNN, contract names, issue #NN).
