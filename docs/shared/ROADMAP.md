...
- **Most of this follow-on work has now shipped** in the
  ...
  see [STATUS.md](STATUS.md) §"Usable intake dimensions"). Status of the original
  three items: (1) **parser/plugin source adaptation for nutrition/supplements** —
  the *path* is shipped (the intake parser contract + the `parser-generator`
  skill's intake path + a reference parser/fixture proving `parse → IntakeBatch →
  persist`); what remains is adapting a *specific real vendor export*, now
  fill-in-the-blank parser work, **not** a built-in importer; (2) **concrete
