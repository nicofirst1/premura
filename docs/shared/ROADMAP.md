...
5. **Loggable session substrate — in progress.** The
   `session-log-substrate-01KT45S1` mission (slice one) is landing the per-run
   **session log** (a local, PHI-bearing DuckDB store recording sessions, steps,
   and ingest provenance) plus the runtime-contract checker, sandbox/ingest
   runner, reference parsers + fixtures, deterministic grader, and an offline
   repeatable check — the substrate a future end-to-end agent-acceptance harness
   reads. Its first landed change is a **doctrine clarification** (FR-130): the
   settled runtime build-and-use parser rule — an agent may build a parser and use
   it immediately for the operator's own data with no reviewer; only contributing
   it back as a public PR is reviewed — is now stated consistently across
   `operating-agent-roles.md`, ADR 0010, and [DOCTRINE.md](DOCTRINE.md), and pinned
   by a test (see [STATUS.md](STATUS.md) §"Runtime build-and-use parser boundary").
   The rest of the slice is implemented on the mission lane and not yet merged.
