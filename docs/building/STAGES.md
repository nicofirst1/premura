...
- **Signal-backed** — one `signal(name, params)` tool exposing every grounded signal (resting HR status and trend, steps trend, weight trend, deep-sleep baseline, HRV change, supplement adherence, nutrition-intake trend, …); `signal()` with no name returns a self-describing catalog. Each answer opens the warehouse read-only and delegates to the Stage 2 engine; the `status` field (`available` / `missing_input` / `stale_input` / `insufficient_data`) keeps each refusal reason distinct.
...
- **Analytical** — the engine's closed set (`engine.list_analytical_tools()`) exposed as three tools: `analyze(method, …)` for the descriptive single-metric patterns (change-point, smoothed average, rolling mean), `paired_test(kind, …)` for the pre-registered before/after and condition-paired differences (a declared paired difference with a descriptive uncertainty band — **not** a significance test), and `correlate` for a pre-registered, caller-declared lagged association (no p-value, names no cause). Each delegates to the engine over the bounded analytical contract, returns the mandatory result envelope (estimate + validity metadata + closed confound checklist) or a first-class refusal, and takes an optional `session_id` recording the call in a research trace.
