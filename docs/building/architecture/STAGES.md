...
> source (adapting a *specific real vendor export* stays parser/plugin
> follow-on work — though the usable-intake-dimensions mission has since shipped
> the intake **parser path** and a reference parser/fixture, so this is now
> fill-in-the-blank). **Concrete resolvers for the intake domains have since
> shipped** (the usable-intake-dimensions mission): `nutrition_intake` and
> `supplement_intake` now resolve through the seam, and the `unsupported_domain`
> outcome is gone — a no-row case returns an explicit missing/stale outcome and
> is never coerced into observations. Further profile-dependent Stage 2 answers
> beyond BMI and the two intake signals remain follow-on (age-adjusted
> interpretation is the next deferred candidate). Do not infer a built-in
> nutrition importer from the existence of the intake tables.
...
- **Stage 2 (Signal processing)** may *read* profile/intake context to answer a
  ...
  `nutrition_intake` and `supplement_intake` are now **resolvable declaration
  targets** with concrete resolvers and two descriptive signals
  (`supplement_intake_adherence`, `nutrition_intake_trend`); a no-matching-row
  case resolves to an explicit missing/stale outcome and is never coerced into
  observations.
