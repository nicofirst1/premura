...
> Status: live contract. The stable structured surface the research trace audit
> skill (`src/premura/skills/research-trace-audit/`) reads. Authored by the
> `session-research-trace-01KSYT4A` mission; moved here from that mission's
> `kitty-specs/.../contracts/` bookkeeping on 2026-06-11 so its only home is a
> live doc. See design decision note
