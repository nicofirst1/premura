...
3. **Then literature grounding (deferred).** PubMed search/fetch and the literature-to-warehouse bridge belong after the deterministic tool layer exists, so citations attach to tool-grounded analysis rather than free-form narration. Not shipped — a following mission.
4. **Then reproducible research traces / the multiplicity audit (deferred).** Analytical sessions should leave behind a markdown trace of tool calls, outputs, and caveats; the same session-scoped trace is where per-session multiplicity accounting (the "1 notable result among 47 examined" disclosure design decision note `0008` pushes out of the stateless engine) lands. Not shipped — a following mission.
