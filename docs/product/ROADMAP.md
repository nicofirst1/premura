...
2. **The first deterministic tools — first slice shipped.** Two conservative, reproducible proof tools landed on the default MCP surface: `change_point` (level-shift detection) and `smoothed_average` (trailing smoothed pattern), each routed to its own first-class analytical `QuestionType` and returning the mandatory result envelope (estimate + validity metadata + confound checklist, or a first-class refusal). **Still open:** `correlate` (the next and highest-value tool — the n-of-1 workhorse), `rolling_mean`, `paired_t_test`, and only later broader significance-testing coverage. The goal stays honest n-of-1 analysis, not statistical theater.
