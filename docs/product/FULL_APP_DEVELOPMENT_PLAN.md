...
- a real Stage 3 MCP surface — a default agent surface (`premura-mcp`) of thirteen validity-gated tools (`list_metrics`, `metric_summary`, six signal-backed tools, two agent-mediated profile-capture tools, and the analytical tools `change_point` / `smoothed_average` / `correlate`), plus a separate operator surface (`premura-mcp-operator`) that adds the raw `query_warehouse` escape hatch
...
- the remaining deterministic Stage 3 statistics tools (`paired_t_test`, `rolling_mean`), PubMed integration, and the signal selector — `change_point`, `smoothed_average`, and `correlate` have since shipped on the default surface over the bounded analytical contract; the rest this first grounded slice did not build
...
> tools now sit on the default MCP surface — `change_point`, `smoothed_average`,
> and `correlate` (the first multi-input tool: a pre-registered, caller-declared
> whole-day-lagged Spearman *association* with an autocorrelation-corrected
> `N_eff` band, never a p-value or "significant") — each carrying the mandatory
> result envelope (estimate + validity metadata + closed confound checklist,
> including the `common_cause_plausible` key) or a first-class refusal. See
...
> the reproducible research trace / session-scoped multiplicity audit — each a
> following mission, none shipped yet.
