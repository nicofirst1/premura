...
- `docs/product/VISION.md` — long-term pillars (extensibility, teaching, privacy); explicitly labeled "proposal/archive, not a build contract."
...
10. **Local-first privacy posture is non-negotiable** and inherited by every stage: cleartext never leaves the machine unencrypted, no telemetry, network only on explicit user action at Stage 3 (`docs/product/VISION.md` Pillar 6; `docs/architecture/STAGES.md` §3).
