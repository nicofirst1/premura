...
```bash
uv run python -c "
from premura import engine, mcp, learn
...
stubs = [
    ...
    ('learn.start_interview', lambda: learn.start_interview()),
...
```bash
uv run ruff check src/premura/{engine,mcp,learn,skills,parsers/_lang,parsers/lookup.py,parsers/base.py}
uv run mypy src/premura/{engine,mcp,learn,skills,parsers/_lang}
```
...
5. **Interview / Learn mission** — implement `start_interview` and the 6 health-direction tracks; depends on MCP being operational.
