...
- New subpackages under `src/premura/`:
  ...
  - **`learn/`** (Stage 4) — `__init__.py` docstring-only stub with `start_interview()` raising `NotImplementedError`. Docstring documents: drives the 6-direction interview (Pillar 4), hands domains to `mcp.register_tools`. Layering rule: never reads `hp.fact_measurement` or calls `engine` directly.
...
- New `src/premura/parsers/CONTRACT.md` — agent-agnostic parser contract. Documents (a) the `PluginParser` Protocol, (b) the standards-first decision tree (existing alias → LOINC for labs → IEEE 1752.1 for wearables → vendor-namespaced), (c) the `dim_metric.yaml` row shape, (d) the `unmapped_metrics` and alias-PR-with-parser feedback loops, (e) the **reserved `derived:*` namespace** rule (community parsers must not emit `derived:*` — only the engine layer does).
...
- Extended `src/premura/dim_metric.yaml` to ~150 rows total (target 150 ±10, minimum hard floor 140). Existing 43 rows gain a `category` field. New ~107 rows cover: wearable expansion (~40), CBC + chemistry + lipids + liver/kidney + electrolytes + thyroid + iron + vitamins + inflammation + endocrine (~80), urine starter (~15), stool starter (~15). English canonical `metric_id`s; LOINC + IEEE-1752.1 cross-references where available; aliases in flat per-language buckets (`en`, `it`, `de`, …) with vendor field names stored in `en:`.
...
| FR-009 | The system SHALL contain `src/premura/parsers/CONTRACT.md` — agent-agnostic — covering the `PluginParser` Protocol, the standards-first decision tree (existing alias → LOINC for labs → IEEE 1752.1 for wearables → `vendor:<source>:<field>` fallback), the `dim_metric.yaml` row shape including aliases, the alias-PR feedback loop, and the **reserved `derived:*` namespace** rule. | The file exists; a test asserts the strings `"LOINC"`, `"IEEE 1752.1"`, and `"derived:"` all appear in its content. | Draft |
...
| NFR-003 | New Python code SHALL pass `ruff check` and `mypy` under the existing project config. | `uv run ruff check src/premura/{engine,mcp,learn,skills,parsers}` and `uv run mypy src/premura/{engine,mcp,learn,skills,parsers}` exit 0. | Draft |
...
| C-002 | No file under `src/premura/parsers/{health_connect,garmin_gdpr,sleep_as_android,bmt}.py`, `src/premura/{encrypt,notify,upload,dedupe,loader,config}.py`, or existing tests under `tests/test_parsers/` MAY be modified by this mission. The existing `Parser` Protocol, `Measurement`, `Interval`, `ParseResult` symbols in `parsers/base.py` MUST remain unchanged. | v1 close-out is owned by the parallel agent ([V1_CLOSEOUT.md](../../docs/V1_CLOSEOUT.md)); scope discipline avoids merge conflicts. | Draft |
...
| C-005 | No row MAY be removed from `dim_metric.yaml`. Rows MAY only be added or have new columns populated. | Existing facts in the warehouse reference these `metric_id`s. | Draft |
...
| C-012 | The `mcp/` module SHALL NOT import from `premura.store` or call DuckDB directly. The `learn/` module SHALL NOT import from `premura.store`, `premura.engine`, or call DuckDB directly. These layering boundaries are documented in module docstrings; an optional future mission may add import-graph linting. | STAGES.md layering rule: each stage reads only through the layer below it. | Draft |
...
3. The contributor browses `src/premura/{engine,mcp,learn,parsers}` and reads docstrings explaining the architectural intent of each layer.
...
3. Claude reads `src/premura/parsers/CONTRACT.md` (which the skill points to). For each vendor field, Claude follows the standards-first decision tree:
   ...
   - If still miss → propose `vendor:<source>:<field>`.
...
1. User opens an MCP-aware client and tells Learn: *"I'm worried about my liver."*
2. Learn calls `mcp.register_tools(server, domains=["liver"])`.
...
- **`SignalSpec` dataclass** — new in `src/premura/engine/_registry.py`. 7 fields: `name`, `domain`, `inputs`, `output`, `priority`, `auto_safe`, `revision`, plus `fn`. Frozen.
...
- The user's `~/repos/no_git/health_digitalizatino/` repo is reachable from the implementation machine and contains the IT/DE alias maps + canonical-test-name normalisation tables referenced in [FEATURE_BLOOD.md](../../docs/FEATURE_BLOOD.md). If absent at implementation time, the agent generates ≤30 lab rows with English-only names and flags the gap.
