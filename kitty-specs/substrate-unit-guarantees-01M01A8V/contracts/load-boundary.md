...
3. **Vocabulary rail**: `validate_batch_against_warehouse` raises `ValueError` before any write when `batch.source_kind` is not a registered parser source kind or `MANUAL_LOAD_SOURCE_KIND`.
