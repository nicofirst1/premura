...
- 2026-05-26T13:18:27Z – claude:opus:implementer:implementer – shell_pid=35810 – Cross-file note (NOT in WP01 owned files): tests/test_skeleton.py::test_engine_registry_stays_lazy_until_runtime_helpers_load_builtins calls REGISTRY.clear() WITHOUT resetting the new premura.engine._BUILTINS_LOADED flag. Under the old 'if REGISTRY: return' loader, clearing REGISTRY forced a reload; with the explicit flag it does not, so that test fails when any prior test (e.g. the pre-existing test_existing_lab_ratio_registrations_stay_valid_without_new_metadata) already loaded built-ins. It passes in isolation. Fix is one line in that non-owned test (add 'premura.engine._BUILTINS_LOADED = False' after REGISTRY.clear()); flagging for reviewer/orchestrator routing since test_skeleton.py is outside WP01 scope and owned by no WP in this mission.
