...
| FR-010 | `runtime_valid` is a **bounded runtime-checkable subset**, explicitly **not** the full parser-review contract (`src/premura/parsers/CONTRACT.md`). For an **observation** scenario it is exactly the clauses the shipped `check_runtime_contract` enforces: (1) no reserved `derived:` namespace emitted, (2) declared metric set equals emitted metric set, (3) every declared metric exists in `dim_metric`, (4) the ingest produced a batch without raising. For an **intake** scenario it is the **analogous** bounded check defined against the intake batch's **real integrity surface** — note `IntakeBatch` has **no** canonical declared/emitted *metric* keys (no `declared_metrics` / `emitted_metric_ids` / `dim_metric`), so clauses (1)–(3) have **no intake counterpart by design**: (a) the parser imports and `parse()` returns an `IntakeBatch` (or `ParseOutput` carrying one) **without raising**; (b) `IntakeBatch.validate()` passes — every event `source_id` is covered by a declared `source_descriptor` (this **is** the intake declared-vs-emitted coherence, on the **source** dimension) and there are no duplicate `dedupe_key`s; (c) `persist_intake_batch` loads it without raising. The intake evidence surface for `runtime_valid` is therefore `source_descriptors` / event `source_id` / event `dedupe_key` reconciled by `validate()`, **not** a canonical-metric set. The spec's clause list and the implementation's runtime checker **must agree** (verified in plan). | Draft |
