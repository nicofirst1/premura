...
- **Grounded also means substantive.** A quote must clear a minimum length floor (`premura.harness.judge.MIN_EVIDENCE_QUOTE_CHARS`), not just be a verbatim substring. A 1-2 character quote trivially substring-matches almost any dossier text, so length alone is part of what "grounded" means here; a too-short quote is rejected on the same retry path as a non-verbatim one.
