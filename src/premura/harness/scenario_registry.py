...
Today it lists the observation scenario (WP01), the intake scenario (WP04), and
the garbage_refusal scenario (#51) — the ≥ 2-scenarios surface WP05's structural
test asserts over (SC-003 / NFR-006). The module is intentionally import-light
(each factory is import-light) so the live harness path can iterate scenarios
without pulling in Ollama or the network.
"""
...
from premura.harness.garbage_refusal_strategy import garbage_refusal_scenario
...
def all_scenarios() -> list[Scenario]:
    ...
    (observation first, then intake, then garbage_refusal) so callers that index
    or report over it are deterministic.
    """
    return [observation_scenario(), intake_scenario(), garbage_refusal_scenario()]
