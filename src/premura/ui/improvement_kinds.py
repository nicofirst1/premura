...
:func:`premura.session_log.store.record_improvement_item` validates a
``kind`` against this registry's **live** contents, not a fixed vocabulary —
so a new kind is added by calling :func:`register_kind` (directly, or via
that store function's ``kind_description`` auto-register path), with no
central edit to this module or to the store. The seeded six are examples of
the contract, not a closed persona-style list, exactly like the five
reference roles in :mod:`premura.ui.roles`.
