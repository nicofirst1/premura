...
1. **Agent self-check.** The `human_facing` role applies these criteria to its own draft narration before submitting it to `present_answer` — as an **advisory input to drafting**, never a second gate (the deterministic answer-audit gate stays the only gate; see [`HUMAN_FACING.md`](HUMAN_FACING.md)).
